...
def test_format_simple():
    """
        В строку можно подставлять объекты различных типов
...
def test_format_attributes():
    """
        С помощью плейсхолдеров можно обращаться к элементам списков,
        словарей или аттрибутам объектов
    """
...
def test_format_align():
    """
        Можно дополнять строку различными символами (по умолчанию пробелами)
        до нужного количества и выравнивать
    """
...
def test_format_truncate_align_parametrized():
    """
        В плейсхолдер вместо конкретных значений можно подставлять параметры,
        поименованные или нет
    """
    ...
    str1 = '{:{}{}.{after_dot}f}'.format(
                                        45.34354325, padding_number,
                                        total_len, after_dot=3
                                            )
    assert str1 == '045.344'
